...
export default function Step2ExerciseSelection(props)
{
    const getExercisesForCurrentDay = () => {
        const exerciseDay = props.weekdays.indexOf(props.weekdays.filter(x => x.checked === true)[props.indexDay]);
        return props.selectedExercises.filter(x => x.day === exerciseDay)[0].exercises;
    }

    const handleRemoveExercise = (exerciseId) => {
        const exerciseDay = props.weekdays.indexOf(props.weekdays.filter(x => x.checked === true)[props.indexDay]); //get number day
